...
class AITetrisGame():
    def __init__(self, screen):
        ...
        self.loser_flag = 'AI1_lost.txt'
        self.death_message = "AI 1 LOST!"
        self.player_loser_flag = "player1_lost.txt"
        self.ai2_loser_flag = "AI2_lost.txt"
        self.shared_pause = 0
    ...
    def update_score(self, nscore):
        ...
        with open('AI1_Score.txt', 'w') as f:
            if int(score) > nscore:
                f.write(str(score))
            else:
                f.write(str(nscore))
    ...
    def evaluate_position(self, grid, piece_positions):
        ...
        score = (
                ...
                0.5 * self.calculate_wells(heights)  # Reward good well setups
        )
    ...
    def max_score(self):
        try:
            with open('AI1_Score.txt', 'r') as f:
                lines = f.readlines()
                score = lines[0].strip()
    ...
    def main(self, win):
        ...
        while run:
            ...
            if self.check_lost(self.locked_positions):
                ...
                self.update_score(score)
            if os.path.exists(self.ai2_loser_flag) or os.path.exists(self.player_loser_flag):
                pygame.mixer.music.stop()
