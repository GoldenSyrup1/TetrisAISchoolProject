...
class TetrisGame():
    ...
    def __init__(self, screen):
        ...
        self.loser_flag = "player2_lost.txt"  # Changed from loser_flag1.txt
        self.death_message = "PLAYER 2 LOST!"  # Specific message for player 2
        self.player_loser_flag = "player1_lost.txt"
        ...
        self.setup_dimensions=(1.0)  # Initialize with scale 1.0
        ...
        try:
            ...
            self.pause_sound = pygame.mixer.Sound("Super Mario Bros.-Pause Sound Effect.mp3")
            self.gameover_sound = pygame.mixer.Sound("Undertale OST_ 011 - Determination-[AudioTrimmer.com].mp3")
        except FileNotFoundError as e:
            print(f"Error loading sound effects: {e}")
            self.clear_sound = self.pause_sound = self.gameover_sound = None
    class Piece(object):
    ...
    def create_grid(self, locked_pos=None):
    ...
    def update_score(self, nscore):
        ...
        with open('multiplayer_scores.txt', 'w') as f:
            if int(score) > nscore:
                f.write(str(score))
            else:
                f.write(str(nscore))
    ...
    def max_score(self):
        try:
            with open('multiplayer_scores.txt', 'r') as f:
                lines = f.readlines()
                score = lines[0].strip()
    ...
    # In TetrisCopyCopy.py, add this method to the TetrisGame class:
    def apply_scaling(self, scale_factor):
        """Scale all game dimensions by the given factor"""
        self.s_width = int(800 * scale_factor)
        self.s_height = int(700 * scale_factor)
        self.play_width = int(300 * scale_factor)
        self.play_height = int(600 * scale_factor)
        self.block_size = int(30 * scale_factor)
        self.top_left_x = (self.s_width - self.play_width) // 2
        self.top_left_y = self.s_height - self.play_height
    ...
    def main(self, win):
        ...
        if os.path.exists(self.loser_flag):  # Changed from loser_flag1.txt
        ...
        while run:
            ...
            if not paused:
                ...
                if not change_piece:
                    ...
                    if keys[pygame.K_w] and (current_time - last_rotation_time) > rotation_delay:
                    ...
                    if keys[pygame.K_e] and (current_time - last_move_time) > 2 * move_delay:
                        self.space_once(current_piece, grid)
                        last_move_time = current_time
                    if keys[pygame.K_q] and can_hold:  # Press 'C' to hold
                ...
                if change_piece:
                    ...
                    lines_cleared, score_increase = self.clear_rows(grid, self.locked_positions)
            ...
            if self.check_lost(self.locked_positions):
                # Game over logic
                with open(self.loser_flag, "w") as f:  # Changed
                    f.write("lost")
                ...
                pygame.display.update()
                try:
                    pygame.mixer.music.load("Undertale OST_ 011 - Determination-[AudioTrimmer.com].mp3")
                except pygame.error as e:
                    print(f"Error loading music: {e}")
                ...
                run = False
                self.update_score(score)
            if os.path.exists(self.player_loser_flag):
                pygame.mixer.music.stop()
